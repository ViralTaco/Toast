...
public:
  ...
  Element(bool is_checked, const std::string& content) noexcept 
    : is_checked_(is_checked), content_(content)
    , mark_((is_checked) ? " [\x1b[92m*\x1b[0m] " : " [] ") 
  {}
  
  Element(const std::string& content) noexcept 
...
public:
  Bread(const std::string path) noexcept : kPath_(path) 
  {
    set_elements();
  }
...
public:
  ...
  void check(const size_t& id) noexcept 
  {
    elements_.at(id).check();
  }
  
public:
  size_t get_bread_count() const noexcept 
  {
  ...
  std::string to_string(const size_t index) const
  {
    return elements_.at(index).to_string();
  }
  
  std::pair<bool, std::string> at(const size_t& index) const 
  {
    const auto e = elements_.at(index);
    return std::make_pair(e.is_checked_, e.content_);
  }

  std::pair<bool, std::string> operator[](const size_t& index) const
  {
